 ...
 */
contract Rip7560SimpleAccount is Rip7560BaseAccount, UUPSUpgradeable, Initializable {
    using ECDSA for bytes32;
    address public owner;

    address private constant _entryPoint = 0x0000000000000000000000000000000000007560;

    event Rip7560SimpleAccountInitialized(address indexed entryPoint, address indexed owner);
    ...
    function _requireFromEntryPointOrOwner() internal view {
        require(msg.sender == _entryPoint || msg.sender == owner, "account: not Owner or EntryPoint");
    }

    /// implement template method of BaseAccount
    function _validateSignature(bytes calldata transaction, bytes32 txHash)
        internal
        virtual
        override
